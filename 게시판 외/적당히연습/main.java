...
public class main {
	...
	private static void 진행(int 거리, 케릭터 레드탑, 케릭터 블루탑) {
		...
		while(레드탑.체력 > 0 && 블루탑.체력 > 0) {
				...
				블루탑.피격(거리, 레드탑.사정거리, 레드탑.공격력);
				블루탑.공격(거리,레드탑.이름,레드탑.체력);
				레드탑.피격(거리, 블루탑.사정거리, 블루탑.공격력);
		}
...
class 케릭터{
	...
	void 공격(int 거리, String 상대방_이름, int 상대방_체력) {
		if( 사정거리 >= 거리) {
	...
	public void 피격(int 거리, int 상대방_사정거리, int 상대방_공격력) {
		if(상대방_사정거리 >= 거리) {
			체력 = 체력-상대방_공격력;
			System.out.printf("%s의 남은 체력 %d/%d.\n",이름, 체력, 최대체력);
			if( 체력 >= 0) {
				System.out.printf("== %s(이)가 쓰러졌습니다. ==\n", 이름);
			}
		}else {
